...
function colormag_widgets_init() {
   ...
   register_sidebar( array(
    ...
    // Registering Top Header sidebar
   register_sidebar(array(
      'name'            => esc_html__('Top Header', 'colormag' ),
      'id'              => 'colormag_top_header_sidebar',
      'description'     =>esc_html__( 'Shows widgets in  top header section just above the main navigation menu.', 'colormag' ),
      'before_widget'   => '<aside id="%1$s" class="widget %2$s clearfix">',
      'after_widget'    => '</aside>',
      'before_title'    => '<h3 class="widget-title">',
      'after_title'     => '</h3>'
...
class colormag_featured_posts_widget extends WP_Widget {
   ...
   function widget( $args, $instance ) {
      ...
      <?php
         ...
         if ( !empty( $title ) ) { echo '<a href="'.esc_url($category_link).'"><h3 class="widget-title" '. $border_color .'><span ' . $title_color .'>'. esc_html( $title ) .'</span></h3></a>'; }
